...
sox $1 -t raw -b 16 --encoding signed-integer -c 1 -r 16000 $workdir/raw.temp norm -3

cat $workdir/raw.temp | $sptk/bcut +s -s $3 -e $4  | $sptk/x2x +sf > $workdir/rawinput.temp
...
$sptk/x2x +fa | awk '{if ($1 == 0) {print "-1"} else {print log($1)}}' | $sptk/x2x +af \
> $workdir/pitch.temp
...
echo "`$sptk/x2x +fa30 $workdir/all_feat.temp | wc -l` frames in output!" 1>&2
